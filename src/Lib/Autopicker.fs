...
type Compose() =
    let pickOne create item =
        item |> Option.bind create
    ...
    member _.ctor2 (ctor: _ -> 'ctor) (choice1: ComposedChoice<'acc,'arg1,_>) (choice2 : ComposedChoice<'acc,'arg2,_>) : ComposedChoice<'acc,'ctor,'r> = fun create acc ->
        if rand.Next 10 < 3 then
            ctor(1,2) |> Some |> (pickOne create)
        elif rand.Next 10 < 3 then
            let choicex = choice1
            let choice1 = notImpl()
            choice1 (Option.bind (fun arg1 -> ctor(1, 2) |> Some |> pickOne create))
        else
            notImpl()
            ...
            //    | Some arg1 -> choice2 (function arg2 -> create2 arg1 arg2)
            //    | None -> fun acc -> None |> create
            //choice1 create1
